...
export default function NotasDiretorPage() {
  ...
  // Carrega turmas e disciplinas quando curso muda
  useEffect(() => {
    if (selectedCourse) {
      loadClasses()
      loadDisciplines()
  ...
  // Carrega alunos e notas quando turma e disciplina são selecionadas
  useEffect(() => {
  ...
  async function loadStudentsAndGrades() {
    ...
    try {
      ...
      // 2️⃣ Buscar notas existentes usando o endpoint simplificado
      let grades: any[] = []
      try {
      ...
      const processedRows: StudentRow[] = students.map((student: any) => {
        ...
        const p1 = p1Grade?.grade
        const p2 = p2Grade?.grade
        const final = finalGrade?.grade
  ...
  const saveGrades = async () => {
    if (!selectedCourse || !selectedClass || !selectedDiscipline) {
      setError("Selecione curso, turma e disciplina antes de salvar.")
      return
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Lançamento e consulta de notas por disciplina</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {error && (
            <Alert variant={error.includes("Salvo com alguns erros") ? "default" : "destructive"}>
          ...
          {loading ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-blue-700">
                        {rows.length} aluno(s) • {rows.filter(r => r.status === "APROVADO").length} aprovado(s) • {rows.filter(r => r.status === "REPROVADO").length} reprovado(s)
                      </p>
                    </div>
            ...
            <div className="text-center py-12 border rounded-lg bg-gray-50">
              <Calculator className="h-12 w-12 text-gray-400 mx-auto mb-3" />
              <p className="text-gray-600 mb-2">Selecione um curso, turma e disciplina</p>
              <p className="text-sm text-gray-500">
                1. Escolha um curso<br />
                2. Selecione uma turma<br />
                3. Escolha uma disciplina<br />
